...
class Cal_SIGA(object):
    def __init__(self, Param_Min, Param_Max, ProjectPath, JSONPath, NameSIGAFolder,
                 ...
                 TypeFobjCal, PathExeSIGA, NameSIGAModel='Hy', NameFunObj='NASH', BaseCSVPath=None):
        """
        ...
        self.S_S4           = 1
        # Ruta base para archivos CSV (externa o interna)
        self.BaseCSVPath    = BaseCSVPath
    ...
    def objectivefunction(self, simulation, evaluation):
        # Calcular la función objetivo (sin penalización del Tanque 4)
        return OF(self.OmitObs,self.FacObjF,self.TS_Sim,self.TS_Obs,self.TypeFobjCal, self.NameFunObj)
...
def simulationSIGA(Params, ProjectPath, JSONPath, NameSIGAFolder, NameSIGAModel,
                   NameVarSIGA,NameSceExe, NameExe, PathExeSIGA, NameCal, TS_Obs, BaseCSVPath=None):
    ...
    if NameSIGAModel == 'Hy':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='Hy', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'Sed':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='Sed', BaseCSVPath=BaseCSVPath)
    elif (NameSIGAModel == 'WQ_NT') | (NameSIGAModel == 'WQ_NO') | (NameSIGAModel == 'WQ_NH4') | (NameSIGAModel == 'WQ_NO3'):
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[:-2], TypeModel='N', BaseCSVPath=BaseCSVPath)
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[-2:], TypeModel='Theta', Posi=[2, 3], BaseCSVPath=BaseCSVPath)
    elif (NameSIGAModel == 'WQ_PT') | (NameSIGAModel == 'WQ_PO') | (NameSIGAModel == 'WQ_PI'):
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[:-2], TypeModel='P', BaseCSVPath=BaseCSVPath)
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[-2:], TypeModel='Theta', Posi=[5, 6], BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_DBO':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[:-1], TypeModel='DBO', BaseCSVPath=BaseCSVPath)
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[-1], TypeModel='Theta', Posi=[0], BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_OD':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[:-1], TypeModel='OD', BaseCSVPath=BaseCSVPath)
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[-1], TypeModel='Theta', Posi=[4], BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_EColi':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[:-1], TypeModel='EColi', BaseCSVPath=BaseCSVPath)
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params[-1], TypeModel='Theta', Posi=[1], BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_CT':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='CT', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_Tur':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='Tur', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_SST':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='SST', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_T':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='T', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_Co':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='Co', BaseCSVPath=BaseCSVPath)
    elif NameSIGAModel == 'WQ_pH':
        Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='pH', BaseCSVPath=BaseCSVPath)

    # Crear archivo de calibración en formato SIGA
    Create_CalibrationFile(ProjectPath, JSONPath, NameSIGAFolder, NameCal, BaseCSVPath=BaseCSVPath)
    ...
    if NameSIGAModel == 'Hy':
        ...
        # NOTA: Se comenta el factor S_S4 porque estaba causando problemas al multiplicar por 0
        # Si se quiere penalizar la pendiente del tanque 4, descomentar la siguiente línea
        # S_S4 = 1 + abs(slope)
        # Por ahora, mantener S_S4 = 1 para todos los modelos
        S_S4 = 1

    return TS_Sim, S_S4, TS_S4

def OF(OmitObs,FacObjF,TS_Sim,TS_Obs,TypeFobjCal,NameFunObj):
    """
    Esta función calcula la métrica de desempeño para la calibración del modelo.

    IMPORTANTE: Esta función devuelve el valor TRANSFORMADO para optimización interna.
    Para obtener el valor ORIGINAL de la métrica (lo que el usuario espera ver),
    usar la función OF_original().

    Parameters
        OmitObs:
                Número de datos iniciales que se omiten para la estimación de la métrica
        FacObjF:
                Factor multiplicador para cambiar el sentido de la métrica de calibración.
                -1 Maximización (para DDS) | 1 Minimización (para LHS, SCE)
        TS_Sim:
                Valores simulados por el modelo
        TS_Obs:
                Valores observados
        TypeFobjCal:
                Tipo de métrica a estimar. 1: t-Student, 0: Serie de tiempo
        NameFunObj:
                Nombre de la función objetivo

    Returns:
        Valor transformado para optimización (usado internamente por SPOTPY)
    """
    ...
    # Retornar valor promediado entre todas las estaciones
    return ObjFun / len(TS_Sim.columns)


def OF_original(OmitObs, TS_Sim, TS_Obs, TypeFobjCal, NameFunObj):
    """
    Calcula la métrica de desempeño en su forma ORIGINAL (sin transformación).
    Esta es la versión que el usuario debe ver en los resultados.

    Parameters
        OmitObs: Número de datos iniciales que se omiten
        TS_Sim: Valores simulados por el modelo
        TS_Obs: Valores observados
        TypeFobjCal: Tipo de métrica (0: Serie de tiempo, 1: t-Student)
        NameFunObj: Nombre de la función objetivo

    Returns:
        Valor original de la métrica (como se conoce en la literatura)
    """
    n = OmitObs
    ObjFun = 0
    ...
    for i in range(len(TS_Sim.columns)):
        ...
        if TypeFobjCal == 0:
            if NameFunObj == 'NASH':
                ...
                # Valor original de COMPOS (menor es mejor)
...
def Modify_csvHyParms(ProjectPath, JSONPath, NameSIGAFolder, Params, TypeModel='Hy',Posi=[], BaseCSVPath=None):
    """
    ...
    Parameters
        ...
        BaseCSVPath:
                Ruta base para archivos CSV (si None, usa la ruta por defecto)
    """
    ...
    # Si se proporciona BaseCSVPath, usarlo; de lo contrario, usar la ruta por defecto
    if BaseCSVPath:
        PathFile = os.path.join(BaseCSVPath, 'calibracion', FileName)
    else:
        # Usar la ruta por defecto (Step_01/archivos_csv)
        current_dir = os.path.dirname(__file__)
        parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir, os.pardir))
        subplugin1_dir = os.path.join(parent_dir, 'Step_01')
        PathFile = os.path.join(subplugin1_dir, 'archivos_csv', 'calibracion', FileName)
...
def Modify_csvMuestreador_CP( JSONPath, NameSIGAFolder, Catalog, NameSIGAModel, X, Y, BaseCSVPath=None):
    """
    ...
    Parameters
        ...
        BaseCSVPath:
                Ruta base para los archivos CSV (opcional). Si no se proporciona, usa la ruta interna.
    """
    ...
    if BaseCSVPath:
        # Usar la ruta externa proporcionada
        PathFile = os.path.join(BaseCSVPath, 'muestreador', FileName)
    else:
        # Usar la ruta por defecto (Data/archivos_csv_Cal)
        current_dir = os.path.dirname(__file__)
    ...
    ControlPoints['Estadistico'] = 'media'
    ControlPoints.iloc[-1, -1] = 'media'

    df_duplicated = pd.concat([ControlPoints] * 2, ignore_index=True)
    df_duplicated.loc[len(ControlPoints):, 'Estadistico'] = 'valor'

    # Guardar archivo
    df_duplicated.to_csv(PathFile,index=False)


def Create_CalibrationFile(ProjectPath, JSONPath, NameSIGAFolder, NameCalibration, BaseCSVPath=None):
    """
    ...
    # Si se proporciona BaseCSVPath, usarlo; de lo contrario, usar la ruta por defecto
    if BaseCSVPath:
        CsvParamPath = os.path.join(BaseCSVPath, 'calibracion' + os.path.sep)
    else:
        # Usar la ruta por defecto (Step_01/archivos_csv)
        current_dir = os.path.dirname(__file__)
        parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir, os.pardir))
        subplugin1_dir = os.path.join(parent_dir, 'Step_01')
        CsvParamPath = os.path.join(subplugin1_dir, 'archivos_csv', 'calibracion' + os.path.sep)
...
def Create_SamplerFile(ProjectPath, JSONPath, NameSIGAFolder, NameSampler, BaseCSVPath=None):
    """
    ...
    if BaseCSVPath:
        # Usar la ruta externa proporcionada
        CsvParamPath = os.path.join(BaseCSVPath, 'muestreador' + os.path.sep)
    else:
        # Usar la ruta por defecto (Data/archivos_csv_Cal)
        current_dir = os.path.dirname(__file__)
        parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir))
...
def ReadTimeSeries(FilePathTS):
    ...
    # Reemplazar los valores de datos faltantes con np.nan
    TS.replace(missing_value, np.nan, inplace=True)
...
# FUNCIÓN VIEJA COMENTADA - NO USAR (no funciona en Windows sin Cygwin)
...
def windows_to_cygwin_path(windows_path):
    """
    Convierte una ruta de Windows a formato Cygwin de manera robusta.
    Soporta múltiples unidades (C:, D:, E:, etc.)

    Parameters:
        windows_path: Ruta en formato Windows (ej: C:\\Users\\Nombre o C:/Users/Nombre)

    Returns:
        Ruta en formato Cygwin (ej: /cygdrive/c/Users/Nombre)
    """
    # Asegurar que la ruta esté en el encoding correcto
    ...
    # Convertir todas las posibles unidades de disco a formato Cygwin
    for drive in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
        drive_upper = f'{drive}:'
        drive_lower = f'{drive.lower()}:'
        if path.startswith(drive_upper):
            path = path.replace(drive_upper, f'/cygdrive/{drive.lower()}', 1)
            break
        elif path.startswith(drive_lower):
            path = path.replace(drive_lower, f'/cygdrive/{drive.lower()}', 1)
            break

    # NO hacer conversión de encoding - dejar que bash maneje UTF-8 nativamente
    # La conversión causaba doble encoding cuando se escribía al archivo temporal
    return path
...
def ExeSIGAModel(ProjectPath, NameSIGAFolder, NameExe, PathExeSIGA):
    ...
    # SOLUCIÓN: Usar el ejecutable que está en la carpeta del proyecto (bin/SIGA.exe)
    # Esto evita problemas con caracteres especiales en la ruta de AppData
    # La carpeta bin está en: ProjectPath/bin/SIGA.exe (no dentro de NameSIGAFolder)
    siga_exe_in_project = os.path.join(ProjectPath, 'bin', 'SIGA.exe')

    # Verificar si existe el ejecutable en el proyecto
    if os.path.exists(siga_exe_in_project):
        # Usar el ejecutable del proyecto (evita "ñ" en AppData)
        dir_siga_exe = siga_exe_in_project
        print(f'Usando SIGA.exe del proyecto: {dir_siga_exe}')
    else:
        # Fallback: usar el ejecutable del plugin
        siga_exe = rf'{PathExeSIGA}'.replace('C:', 'C').replace(os.sep, '/')
        current_dir = os.path.dirname(__file__)
        parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir))

        if os.path.isabs(PathExeSIGA):
            dir_siga_exe = os.path.abspath(PathExeSIGA)
        else:
            dir_siga_exe = os.path.join(parent_dir, siga_exe)

        print(f'Usando SIGA.exe del plugin: {dir_siga_exe}')

    # Convertir la ruta de Windows a la ruta de Cygwin
    dir_siga_exe_cygwin = windows_to_cygwin_path(dir_siga_exe)

    # Construir el comando de ejecución - convertir a formato Cygwin
    path_eje = windows_to_cygwin_path(os.path.join(ProjectPath, NameSIGAFolder))

    # Definir la ruta del archivo de log - convertir a formato Cygwin
    ErrorFile = windows_to_cygwin_path(os.path.join(ProjectPath, NameSIGAFolder, 'Log_Calibration.txt'))

    # Comando con tee para guardar log pero permitir ver la ventana emergente
    # Entrecomillar las rutas que pueden contener espacios
    comando = f'. /etc/profile; "{dir_siga_exe_cygwin}" -t "{path_eje}" -n {NameExe} -a true 2>&1 | tee "{ErrorFile}"'

    print(f'IMPORTANTE - Comando a ejecutar: {comando}')
    ...
    # Buscar bash.exe
    if os.path.exists('C:/cygwin64/bin/bash.exe'):
        bash_path = 'C:/cygwin64/bin/bash.exe'
    else:
        bash_path = 'C:/cygwin/bin/bash.exe'

    # NO capturar stdout/stderr - esto permite que SIGA.exe muestre su ventana emergente
    p = subprocess.Popen([bash_path, '-c', comando])
    p.wait()
