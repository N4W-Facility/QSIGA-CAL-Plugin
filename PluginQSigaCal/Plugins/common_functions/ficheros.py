...
def copiar_archivo(origen, destino):
    os.makedirs(destino+ 'bin/', exist_ok=True)
    try:
        # Verifica si el archivo existe en la ruta origen
        if os.path.exists(origen):
            # Copia el archivo al directorio destino
            shutil.copy2(origen, destino+ 'bin/')
...
def diccionario_factores_calibracion(dic_archivos,ruta_csv):
    ...
    for i in dic_factores_calibracion.keys():
        if dic_factores_calibracion[i].endswith('.csv'):
            archivo_path = os.path.join(ruta_csv, dic_factores_calibracion[i])
            df  = pd.read_csv(archivo_path, encoding='utf8')
            print('Se leyó archivo: ' + archivo_path)
            if i == 'Restauracion':
                n_rest = len(df)
...
def diccionario_muestreador(dic_archivos,ruta_csv):
    ...
    for i in dic_muestreador.keys():
        if dic_muestreador[i].endswith('.csv'):
            archivo_path = os.path.join(ruta_csv, dic_muestreador[i])
            ...
            if df.shape[0]==0 :
                ...
                # df_string = df.to_string(index=False)
                # df_string = '\n'.join(' '.join(line.split()) for line in df_string.split('\n'))
                df_string = df.to_string(index=False, float_format=lambda x: f'{x:.6f}')
                df_string = '\n'.join(' '.join(line.split()) for line in df_string.split('\n'))
...
def diccionario_series(df,df_metadatos,datos_faltantes_series=-9999):
    ...
    # Función auxiliar para buscar columnas case-insensitive
    def find_column(df, column_name):
        """Busca una columna ignorando mayúsculas/minúsculas"""
        for col in df.columns:
            if col.lower() == column_name.lower():
                return col
        raise KeyError(f"Columna '{column_name}' no encontrada en el DataFrame (buscado: {column_name}, {column_name.upper()}, {column_name.lower()})")

    # Buscar nombres de columnas reales (case-insensitive)
    col_codigo = find_column(df_metadatos, 'codigo')
    col_x = find_column(df_metadatos, 'x')
    col_y = find_column(df_metadatos, 'y')
    col_z = find_column(df_metadatos, 'z')

    df           = df.sort_index(axis=1)
    df_metadatos = df_metadatos.sort_values(by=col_codigo).reset_index(drop=True)

    dic_series = {'num_ser' : str(np.shape(df)[1]),
                   'num_per' : str(np.shape(df)[0]),
                   'datos_faltantes' : str(datos_faltantes_series),
                   'x'       : ' '.join(df_metadatos[col_x].values.astype(str)),
                   'y'       : ' '.join(df_metadatos[col_y].values.astype(str)),
                   'z'       : ' '.join(df_metadatos[col_z].values.astype(str)),
                   'datos'   : df.round(4)
                  }
...
class Ficheros(dict):
    ...
    def crear_archivo_vertimientos(self, dic_df, nombre_proyecto=''):
        ...
        nombre_archivo = f'Dumpings_{nombre_proyecto}.txt' if nombre_proyecto else 'Dumpings_.txt'
        escribe_archivo(self.ruta_cuenca_SIGA +'vertimientos/' + nombre_archivo, encabezados, dic_vertimientos)
        print(f'El archivo {nombre_archivo} se creó correctamente')
    def crear_archivo_ciclo_cargas(self, dic_ciclos):
        ...
        escribe_archivo(self.ruta_cuenca_SIGA +'ciclos/LoadCycle.txt', encabezados, dic_ciclos)
        print('El archivo LoadCycle.txt se creó correctamente')
    ...
    def crear_archivo_condiciones_frontera(self,dic_df):
        ...
        escribe_archivo(self.ruta_cuenca_SIGA + 'condicionesfrontera/'+ 'CF_BoundaryCondition.txt',encabezados,dic_vertimientos)
        print('Aviso de crear_archivo_vertimientos: El archivo '+'CF_BoundaryCondition.txt'+' se creó correctamente')
