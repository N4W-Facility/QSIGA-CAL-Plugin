...
def resample_to_match3(input_raster, reference_raster, output_raster, integer_output=False):
        ...
        with rasterio.open(input_raster) as src:
            with rasterio.open(output_raster, 'w', **kwargs) as dst:
                for i in range(1, src.count + 1):
                    # Leer la banda y reescalar
                    data = src.read(i)
                    # Reemplazar valores -1.7976931e+308 por 0
                    try:
                        data[data == -1.7976931e+308] = 0
                    except:
                        data = data

                    # Reproyectar y escribir la banda en el archivo de salida
                    reproject(
                        source=data,
